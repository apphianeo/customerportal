...
import { RightOutlined, DownloadOutlined, PlusCircleOutlined, InfoCircleOutlined } from '@ant-design/icons'
...
const AREA_2_COUNTRIES =
  'Area 2 includes Area 1, Australia, China (excluding Tibet), Hong Kong, India, Japan, South Korea, Macau, New Zealand, Sri Lanka and Taiwan.'
...
function SectionCard({ id, title, children }: { id: string; title: string; children: React.ReactNode }) {
  return (
    <div
      ...
      style={{ scrollMarginTop: 220 }}
    >
      <div className="border-b border-[rgba(0,0,0,0.09)] px-[24px] py-[16px]">
...
export default function UniTravelPolicyPage({ onNavigateToDashboard, onNavigateToPolicies }: Props) {
  ...
  return (
    <div className="bg-bg-page min-h-full">
      ...
      <div className="sticky top-0 z-10 bg-bg-page">
      ...
      <div className="w-full max-w-[980px] mx-auto px-4 sm:px-6 lg:px-8 pb-8 pt-[24px] flex flex-col gap-[24px]">
        ...
        <SectionCard id="section-policy" title="Policy Details">
          <FieldGrid fields={[
            ...
            { label: 'Destination', value: 'Area 2 (Area 1, Australia, China (excluding Tibet), Hong Kong, India, Japan, South Korea, Macau, New Zealand, Sri Lanka and Taiwan)' },
          ...
          <div className="bg-[#eff6ff] shadow-[0px_1px_2px_rgba(0,0,0,0.05)] rounded-[8px] px-[16px] py-[12px] flex flex-col sm:flex-row gap-[12px] sm:items-center">
            <div className="flex gap-[8px] items-center flex-1 min-w-0">
              <InfoCircleOutlined style={{ fontSize: 16, color: '#005eb8' }} className="shrink-0" />
              <p className="text-[14px] text-[#212121] m-0">Supercharge your plan with exclusive add-ons.</p>
            </div>
            <button
              onClick={() => console.log('Open add-ons')}
              className="flex items-center gap-2 border border-[#005eb8] text-[#005eb8] bg-white h-[32px] px-[16px] rounded-[8px] shadow-[0px_1px_2px_rgba(0,0,0,0.05)] font-medium text-[14px] cursor-pointer hover:bg-[#f0f7ff] transition-colors shrink-0"
            >
        ...
        <SectionCard id="section-contact" title="Contact Details">
          <p className="text-[14px] text-[#949494] m-0">No additional contact details on file.</p>
        </SectionCard>

        <SectionCard id="section-others" title="Others">
          <p className="text-[14px] text-[#949494] m-0">Nothing else to show for this policy.</p>
        </SectionCard>
